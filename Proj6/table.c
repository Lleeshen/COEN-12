 ...
 *
 *	Modified June 8, 2017 by Lyman Shen
 * 
 ...
 *              case constant time.
 *
 *				Modifications is to return a sorted array in getElements
 *				instead of an unsorted array.
 */
 ...
 * Complexity:	O(m)
 *
 * Description:	Allocate and return an array of sorted elements in the set pointed
 *		to by SP.
 */
 ...
 *
 * Description:	Auxillary function for getElements to sort the elements using the
 *		quicksort method
 */
...
{
	if(lo < hi) {
		int ploc = partition(compare, a, lo, hi);
		quicksort(compare, a, lo, ploc-1);
		quicksort(compare, a, ploc+1, hi);
	}
}
 ...
 * Complexity:	O(log n)
 *
 * Description:	Auxillary function for quicksort to sort the elements
 *		in each pass
 */
